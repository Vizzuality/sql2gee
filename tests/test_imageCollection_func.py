...
def test_st_summarystats_from_image_collection_happy_case():
    sql = "SELECT st_summarystats(rast,'b1',false) FROM IDAHO_EPSCOR/TERRACLIMATE limit 10"
    response = SQL2GEE(JsonSql(sql).to_json()).response()
    assert len(response) == 10
    assert len(response[0].keys()) == 1, "Response column count incorrect"
    assert len(response[0]['st_summarystats']) == 14, "Response column count incorrect"
    assert "st_summarystats" in response[0].keys()
    assert "aet" in response[0]['st_summarystats']
    assert "count" in response[0]['st_summarystats']['aet']
    assert "max" in response[0]['st_summarystats']['aet']
    assert "mean" in response[0]['st_summarystats']['aet']
    assert "min" in response[0]['st_summarystats']['aet']
    assert "stdev" in response[0]['st_summarystats']['aet']
    assert "sum" in response[0]['st_summarystats']['aet']
    return
