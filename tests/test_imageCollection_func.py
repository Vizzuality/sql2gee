...
def test_image_collection_group_by_with_geometry_where_condition():
    sql = "select first(slp) as slp, system:index from 'NCEP_RE/sea_level_pressure' where ST_INTERSECTS(ST_SetSRID(ST_GeomFromGeoJSON('{\"type\":\"Point\",\"coordinates\":[-110.22939224192194,19.986126139624318]}'),4326),the_geom) group by system:index limit 10"
    response = SQL2GEE(JsonSql(sql).to_json()).response()
    print(response)
    assert len(response) == 10, f"Empty response: {response}"
    assert "slp" in response[0].keys(), f"Empty response: {response}"
    return
